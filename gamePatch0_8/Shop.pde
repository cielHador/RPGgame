class Shop {
  PImage orangeChar, purpleChar, originalChar, backgroundDeerView, backgroundCastleView, backgroundGrassView, originalBackgroundView;
  ...
  void buyTest() {
    if (rStart == true) {
      ...
      if (shopPageOne == true) {
        ...
        text("Page 2 'g'", width/2+450, height-20/2);
      }
    }
    if (shopPageOne == false && shopPageTwo == true) {
      ...
      text("Page 1 'f'", width/2+450, height-20/2);
      fill(0);
      rectMode(CENTER);
      rect(width/2, height-15/2, 200, 20, 7);
      fill(255);
